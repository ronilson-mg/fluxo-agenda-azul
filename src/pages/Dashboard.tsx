...
export default function Dashboard({ subscription, daysRemaining, userId }: DashboardProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [invResp, clientResp, allInvoicesResp] = await Promise.all([
        supabase
          .from('fa_invoices')
          .select('*')
          .eq('user_id', userId)
      ...
      if (clientResp.data) setClients(clientResp.data);
      ...
      if (allInvoicesResp.data) {
        ...
        const aReceber = allInvoices.filter(i => i.status === 'open' && i.due_date >= todayStr);
        const emAtraso = allInvoices.filter(i => i.status === 'overdue' || (i.status === 'open' && i.due_date < todayStr));
        const recebido = allInvoices.filter(i => i.status === 'paid');

        const totalRecebido = recebido.reduce((acc, i) => acc + i.amount, 0);
        const totalAtraso = emAtraso.reduce((acc, i) => acc + i.amount, 0);
        const totalAVencer = aReceber.reduce((acc, i) => acc + i.amount, 0);
        
        // Health Score
        const totalGeral = totalRecebido + totalAtraso + totalAVencer;
        const healthScore = totalGeral > 0 ? Math.max(0, 100 - (totalAtraso / totalGeral) * 100) : 100;

        const planPrices: Record<string, number> = {
          'trial': 0, 'pro': 69.90, 'business': 97.90, 'premium': 147.90
        };
        const currentPlanPrice = planPrices[subscription?.plano || 'trial'] || 69.90;
        const roi = currentPlanPrice > 0 ? (totalRecebido / currentPlanPrice) : 0;

        setStatsData({
          aReceber: totalAVencer,
          recebidoMes: totalRecebido,
          emAtraso: totalAtraso,
          clientesAtivos: clientResp.data?.length || 0,
  ...
  const generateBarData = () => {
    ...
    allInvoicesState.forEach(inv => {
      ...
      last4Months.forEach(target => {
        if (target.month === invMonth && target.year === invYear) {
          if (inv.status === 'paid') {
            target.recebido += inv.amount;
          } else if (inv.status === 'overdue' || (inv.status === 'open' && inv.due_date < todayStr)) {
            target.atrasado += inv.amount;
          } else {
            target.aberto += inv.amount;
          }
        }
      });
    });
  ...
  return (
    ...
    >
      ...
      {isTrial && (
        <div className="bg-brand-primary/5 border border-brand-primary/20 rounded-2xl p-4 sm:p-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="text-3xl sm:text-4xl font-display font-black text-brand-primary">{daysRemaining}</div>
            <div>
              <h3 className="font-bold text-sm sm:text-base text-brand-text">Dias de trial restantes</h3>
              <p className="text-xs sm:text-sm text-brand-muted">Assine o Pro para continuar usando sem limites.</p>
            </div>
          </div>
          <button 
            ...
            className="w-full md:w-auto bg-brand-primary hover:bg-brand-primary-hover text-white text-xs sm:text-sm font-bold px-6 py-2.5 rounded-xl transition-all shadow-lg shadow-brand-primary/20 flex items-center justify-center gap-2"
          >
      ...
      {/* Victory Counter / ROI Focus */}
      <div className="bg-brand-card border border-brand-border rounded-2xl sm:rounded-[32px] p-6 sm:p-8 relative overflow-hidden group shadow-2xl shadow-black/30 border-t-2 border-t-brand-primary/20">
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 sm:mb-10 relative z-10 gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-brand-primary/20 flex items-center justify-center border border-brand-primary/30 shadow-lg shadow-brand-primary/10">
              <Trophy className="w-5 h-5 text-brand-primary animate-bounce" />
            </div>
            <div>
              <h3 className="text-[10px] sm:text-xs font-black text-brand-primary uppercase tracking-[2px] sm:tracking-[4px] leading-none mb-1">Impacto Financeiro FluxoAzul</h3>
              <p className="text-[9px] sm:text-[10px] text-brand-muted font-bold uppercase tracking-wider">Provando o valor do seu investimento</p>
            </div>
          </div>

          <div className={cn(
            "px-4 py-2 rounded-xl border font-black text-[9px] sm:text-[10px] uppercase tracking-widest flex items-center gap-2 animate-pulse",
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-10 relative z-10">
          <div className="space-y-1 group/item">
            ...
            <p className="text-2xl sm:text-3xl font-display font-black text-brand-primary tracking-tight">
          ...
          <div className="space-y-1">
            ...
            <p className="text-2xl sm:text-3xl font-display font-black text-brand-text/90 tracking-tight">
              {formatCurrency(statsData.recebidoMes)}
            </p>
            <p className="text-[9px] font-bold text-green-500 uppercase tracking-widest flex items-center gap-1">
              <ArrowUpRight className="w-3 h-3" /> Lucratividade Total
            </p>
          </div>

          <div className="space-y-1">
            <p className="text-[10px] font-black text-brand-muted uppercase tracking-[2px]">Custo do plano hoje</p>
            <p className="text-2xl sm:text-3xl font-display font-bold text-brand-muted/50 tracking-tight">
              {formatCurrency(subscription?.plano === 'pro' ? 69.90 : subscription?.plano === 'business' ? 97.90 : subscription?.plano === 'premium' ? 147.90 : 0)}
            </p>
            <p className="text-[9px] font-bold text-brand-muted uppercase tracking-widest">Investimento Mensal</p>
          </div>

          <div className="space-y-1 bg-white/5 p-4 sm:p-5 rounded-[20px] sm:rounded-[24px] border border-white/5 backdrop-blur-sm shadow-xl">
            <p className="text-[10px] font-black text-brand-primary uppercase tracking-[2px]">Lucro Líquido Real</p>
            <p className="text-2xl sm:text-3xl font-display font-black text-brand-text tracking-tight">
            ...
            <div className="mt-2 text-[9px] font-black text-black bg-brand-primary px-3 py-1 rounded-full w-fit uppercase tracking-tighter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-brand-card border border-brand-border rounded-2xl p-6 shadow-xl shadow-black/20">
          <h3 className="font-display font-bold text-brand-text mb-8 uppercase tracking-wide">Fluxo de Caixa</h3>
        ...
        {/* Cobranças por Mês */}
        <div className="bg-brand-card border border-brand-border rounded-2xl p-6 shadow-xl shadow-black/20">
          <h3 className="font-display font-bold text-brand-text mb-8 uppercase tracking-wide">Cobranças por Mês</h3>
      ...
      <div className="bg-brand-card border border-brand-border rounded-2xl sm:rounded-[32px] overflow-hidden shadow-2xl shadow-black/30 border-t-2 border-t-brand-danger/20">
        <header className="p-6 sm:p-8 border-b border-brand-border flex flex-col sm:flex-row items-center justify-between gap-6 bg-gradient-to-r from-brand-danger/5 to-transparent">
          ...
          <div className="w-full sm:w-auto flex flex-col sm:flex-row items-center gap-4">
            ...
            <button className="text-[9px] sm:text-[10px] font-black text-brand-primary hover:text-white uppercase tracking-[2px] transition-colors">
                ...
                <tr key={inv.id} className="hover:bg-brand-bg/50 transition-all group">
                  ...
                  <td className="px-6 sm:px-8 py-6 text-right">
                    <div className="flex items-center justify-end gap-2 sm:gap-3">
                       ...
                       {/* Link to invoices page would be complex without routing params, so we just add the icon and maybe a toast for now or just wait for them to use the Invoices page */}
                       <button 
