...
export default function Dashboard({ subscription, daysRemaining, userId, activeModule, onPageChange, onOpenReferrals }: DashboardProps) {
  ...
  const [showUrgentActionModal, setShowUrgentActionModal] = useState(false);
  const [showPendingConfirmationsModal, setShowPendingConfirmationsModal] = useState(false);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [invResp, clientResp, allInvoicesResp, upcomingAppointmentsResp, allAppointmentsResp, servicesResp, settingsResp] = await Promise.all([
        supabase
          .from('fa_invoices')
          .select('*')
          .eq('user_id', userId)
      ...
      const normalizeApt = (a: any) => {
        const client_name = a.client_name || a.client?.name || finalClients.find((c: any) => c.id === a.client_id)?.name || 'Cliente';
        const client_phone = a.client_phone || a.client?.phone || finalClients.find((c: any) => c.id === a.client_id)?.phone || '';
        const service_name = a.service_name || a.service?.name || finalServices.find((s: any) => s.id === a.service_id)?.name || 'Serviço';
        const price = a.price ?? a.service?.price ?? finalServices.find((s: any) => s.id === a.service_id)?.price ?? 0;
        const prepayment_amount = a.prepayment_amount ?? (price * (a.service?.prepayment_percentage || 0) / 100);
        ...
        return {
          ...
          price,
          prepayment_amount,
          client: a.client || { name: client_name, phone: client_phone },
          service: a.service || { name: service_name, price: price }
        };
      };
      ...
      if (allAppointmentsResp.data && allAppointmentsResp.data.length > 0) {
        appointments = allAppointmentsResp.data.map(normalizeApt);
      } else {
        const localApts = localStorage.getItem(`fa_appointments_${userId}`);
        appointments = (localApts ? JSON.parse(localApts) : []).map(normalizeApt);
      }
      ...
      if (upcomingAppointmentsResp.data && upcomingAppointmentsResp.data.length > 0) {
        upcoming = upcomingAppointmentsResp.data.map(normalizeApt);
      } else {
  ...
  const filteredUpcoming = upcomingAppointments.filter((apt) => {
    ...
    const serviceName = (apt.service?.name || apt.service_name || '').toLowerCase();
  ...
  const getManagementInsight = () => {
    if (activeModule === 'agenda') {
      const pendingSchedules = upcomingAppointments.filter(a => a.status === 'scheduled');
      const pendingCount = pendingSchedules.length;
      if (pendingCount > 0) {
        return {
          badge: "Prevenção de Bolos",
          title: "Confirmação por WhatsApp ativa",
          message: `Você tem ${pendingCount} agendamento${pendingCount > 1 ? 's' : ''} aguardando confirmação. Envie lembretes rápidos de confirmação por WhatsApp agora mesmo para evitar buracos na sua agenda!`,
          buttonText: "Confirmar Horários",
          icon: CalendarCheck,
          action: () => {
            setShowPendingConfirmationsModal(true);
          }
        };
      } else {
        return {
          badge: "Otimização de Agenda",
          title: "Preencha seus horários",
          message: "Excelente! Nenhum agendamento pendente de confirmação para os próximos dias. Deseja abrir a página de agendamentos e registrar novos horários?",
          buttonText: "Agenda Completa",
          icon: Lightbulb,
          action: () => {
      ...
      const overdueInvoices = allInvoicesState.filter(i => i.status === 'overdue' || (i.status === 'open' && i.due_date < todayStr));
      const overdueBalance = overdueInvoices.reduce((acc, i) => acc + (i.amount || 0), 0);
      const overdueCount = overdueInvoices.length;

      const openInvoices = allInvoicesState.filter(i => i.status === 'open' && i.due_date >= todayStr);
      const openBalance = openInvoices.reduce((acc, i) => acc + (i.amount || 0), 0);
      const openCount = openInvoices.length;

      if (overdueCount > 0 || openCount > 0) {
        const totalPendingCount = overdueCount + openCount;
        const totalPendingBalance = overdueBalance + openBalance;
        return {
          badge: "Recuperação Financeira",
          title: "Cobranças Pendentes Ativas",
          message: `Você tem o total de ${totalPendingCount} fatura${totalPendingCount > 1 ? 's' : ''} pendente${totalPendingCount > 1 ? 's (abertas ou vencidas)' : ''} somando R$ ${totalPendingBalance.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}. Acesse o painel filtrando-as para recuperação imediata!`,
          buttonText: "Recuperar Agora",
          icon: Brain,
          action: () => {
            onPageChange('invoices', { filter: 'pending' });
          }
        };
      } else {
        return {
          badge: "Monitoramento Ativo",
          title: "Caixa Planejado",
          message: "Tudo limpo! Nenhuma pendência ou atraso financeiro encontrado. O Fluxo Azul está de prontidão para ajudar você a emitir e controlar suas próximas faturas.",
          buttonText: "Criar Cobrança",
          icon: Zap,
          action: () => {
  ...
  const managementInsight = getManagementInsight();
  const InsightIcon = managementInsight.icon;
  const TipIcon = smartTip.icon;
  ...
  return (
    ...
    >
      {/* Promo Banner Planos */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-brand-card border border-brand-primary/30 p-4 sm:p-6 rounded-[2rem] shadow-lg relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
          <Sparkles className="w-20 h-20 text-brand-primary" />
        </div>
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-brand-primary/10 border border-brand-primary/20 flex items-center justify-center text-xl shrink-0 group-hover:scale-110 transition-transform">
            🚀
          </div>
          <div>
            <h4 className="text-sm font-display font-black text-brand-text uppercase leading-none mb-1">
              {activeModule === 'finance' ? 'Maximize sua produtividade.' : 'Proteja seu caixa.'}
            </h4>
            <p className="text-xs text-brand-muted font-medium">
              {activeModule === 'finance' 
                ? 'Integre o Agenda Azul e nossa IA avançada para automatizar seus agendamentos e garantir que cada horário vire receita.'
                : 'Utilize o Fluxo Azul com IA avançada para automatizar suas cobranças e garantir o recebimento rápido dos seus atendimentos.'
              }
            </p>
          </div>
        </div>
        <button
          ...
          className="w-full sm:w-auto bg-brand-primary hover:bg-brand-primary-hover text-white text-[11px] font-black px-6 py-3.5 rounded-2xl uppercase tracking-widest transition-all shadow-lg shadow-brand-primary/20 shrink-0 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Coluna da Esquerda (Novo Card - Assistente de Gestão) */}
          <div 
            id="management-assistant-card"
            onClick={managementInsight.action}
            className="bg-brand-bg border border-brand-border rounded-[2rem] p-6 relative overflow-hidden group shadow-lg flex flex-col justify-between h-full cursor-pointer hover:border-brand-primary/40 transition-all"
          >
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:scale-110 transition-transform bg-transparent">
              {InsightIcon && <InsightIcon className="w-24 h-24 text-brand-primary/30" />}
            </div>
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-2 h-2 rounded-full bg-brand-primary animate-pulse" />
                  <span className="text-[10px] font-black text-brand-primary uppercase tracking-[2px]">Assistente de Gestão</span>
                </div>
                {/* Badge de contexto */}
                <div className="inline-block bg-brand-primary/15 text-brand-primary text-[9px] font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full mb-3 mt-1">
                  {managementInsight.badge}
                </div>
                <h3 className="text-sm sm:text-base font-display font-black text-brand-text mb-2 leading-snug uppercase italic">
              ...
              <button 
                id="btn-management-insight"
                onClick={(e) => {
                  e.stopPropagation();
                  managementInsight.action();
                }}
                className="mt-6 w-full sm:w-fit bg-brand-primary text-white text-[10px] font-black px-5 py-3 rounded-xl uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-brand-primary/25 flex items-center justify-center gap-2 cursor-pointer border border-transparent"
              >
                {managementInsight.buttonText} <ArrowUpRight className="w-3 h-3" />
              </button>
            </div>
          </div>

          {/* Coluna da Direita (Card de Dica/Feedback original) */}
          <div 
            id="growth-tip-card"
            className="bg-brand-card border border-brand-primary/20 rounded-[2rem] p-6 relative overflow-hidden group shadow-lg flex flex-col justify-between h-full"
          >
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:scale-110 transition-transform">
               {TipIcon && <TipIcon className="w-24 h-24 text-brand-primary/30" />}
            </div>
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                ...
                <h3 className="text-sm sm:text-base font-display font-black text-brand-text mb-2 leading-tight">
                  {smartTip.message}
                </h3>
              </div>
              <button 
      ...
      {/* Victory Counter / Efficiency Focus (ONLY FOR AGENDA MODULE) */}
      {activeModule === 'agenda' && (
        <div className="bg-brand-card border border-brand-border rounded-2xl sm:rounded-[32px] p-5 sm:p-8 relative overflow-hidden group shadow-2xl shadow-black/30 border-t-2 border-t-brand-primary/20">
          <Clock className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 w-48 sm:w-64 h-48 sm:h-64 text-brand-primary opacity-[0.03] group-hover:scale-110 transition-transform duration-1000 group-hover:rotate-12" />
          ...
          <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 sm:mb-10 relative z-10 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-[9px] sm:text-xs font-black text-brand-primary uppercase tracking-[1px] sm:tracking-[4px] leading-tight mb-1">Eficiência Operacional - Agenda Azul</h3>
                <p className="text-[8px] sm:text-[10px] text-brand-muted font-bold uppercase tracking-wider">Provando o valor do seu investimento em produtividade</p>
              </div>
            </div>

            <div className="px-3 py-1.5 rounded-lg border font-black text-[8px] sm:text-[10px] uppercase tracking-widest flex items-center gap-2 animate-pulse w-fit bg-brand-primary/10 text-brand-primary border-brand-primary/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-10 relative z-10">
            <div className="space-y-1 group/item">
              <p className="text-[10px] font-black text-brand-muted uppercase tracking-[2px] group-hover:text-brand-primary transition-colors">Horas economizadas este mês</p>
              <p className="text-xl sm:text-3xl font-display font-black text-brand-primary tracking-tight truncate">
                {(statsData.totalAgendado * 0.5).toFixed(1)}h
              </p>
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black text-brand-muted uppercase tracking-[2px]">Total de atendimentos otimizados</p>
              <p className="text-xl sm:text-3xl font-display font-black text-brand-text/90 tracking-tight truncate">
                {statsData.totalAgendado}
              </p>
              <p className="text-[8px] font-bold text-green-500 uppercase tracking-widest flex items-center gap-1">
                <ArrowUpRight className="w-2 h-2" /> Clientes Agendados
              </p>
            </div>

            <div className="space-y-1">
              <p className="text-[10px] font-black text-brand-muted uppercase tracking-[2px]">Tempo Médio por Setup</p>
              <p className="text-xl sm:text-3xl font-display font-bold text-brand-muted/50 tracking-tight">
                30 Seg
              </p>
              <p className="text-[8px] font-bold text-brand-muted uppercase tracking-widest">Autosserviço de agendamento</p>
            </div>

            <div className="space-y-1 bg-white/5 p-4 sm:p-5 rounded-2xl border border-white/5 backdrop-blur-sm">
              <p className="text-[10px] font-black text-brand-primary uppercase tracking-[2px]">Eficiência de Agenda</p>
              <p className="text-xl sm:text-3xl font-display font-black text-brand-text tracking-tight truncate">
                {statsData.taxaOcupacao.toFixed(0)}%
              </p>
            ...
            <div className="w-full lg:w-auto flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
              <div className="w-full sm:w-auto flex items-center gap-4 bg-brand-primary/10 px-6 py-4 rounded-2xl border border-brand-primary/20 shadow-inner justify-center">
                <div className="flex items-baseline gap-1">
                  <p className="text-2xl sm:text-3xl font-display font-black text-brand-primary">
                    {Math.max(1, statsData.totalAgendado * 3)}x
                  </p>
                  <span className="text-sm font-black text-brand-primary italic">X</span>
                </div>
                <div>
                  <p className="text-[10px] font-black text-brand-primary uppercase tracking-[2px]">Multiplicador de Foco</p>
                  <p className="text-[9px] text-brand-muted font-bold uppercase tracking-tight font-sans">Livre de interrupções de chat</p>
                </div>
              </div>
              ...
              <div className="hidden sm:flex items-center gap-3 bg-brand-bg px-4 py-3 rounded-2xl border border-brand-border h-fit">
                ...
                <p className="text-[9px] font-black text-brand-text uppercase tracking-widest leading-tight">
                  Proteção Automática de Horários<br/>
                  <span className="text-brand-muted">Agenda Organizada</span>
                </p>
              </div>
            </div>
            ...
            <div className="w-full lg:flex-1 lg:max-w-sm">
              <div className="bg-brand-primary text-white p-4 sm:p-5 rounded-2xl sm:rounded-[24px] shadow-2xl shadow-brand-primary/20 relative overflow-hidden group/box">
                ...
                <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-widest leading-relaxed opacity-90">
                  Economizou cerca de <strong className="text-white underline">{(statsData.totalAgendado * 0.5).toFixed(1)} horas</strong> este mês que seriam gastas em ligações e mensagens manuais de confirmação.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className={cn("grid grid-cols-1 gap-6", activeModule === 'agenda' ? "lg:grid-cols-3" : "lg:grid-cols-2")}>
        {activeModule === 'agenda' && (
          ...
          <div className="bg-brand-card border border-brand-border rounded-2xl p-6 shadow-xl shadow-black/20 flex flex-col h-[520px]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-bold text-brand-text uppercase tracking-wide flex items-center gap-2">
                <Clock className="w-4 h-4 text-brand-primary" />
                Próximos Horários
              </h3>
              <span className="text-[8px] font-black bg-brand-primary/10 text-brand-primary px-2 py-1 rounded uppercase tracking-widest">Total: {upcomingAppointments.length}</span>
            </div>
            ...
            <div className="mb-4 relative">
              <input 
                ...
                className="w-full bg-brand-bg/60 border border-brand-border/80 focus:border-brand-primary/50 rounded-xl px-4 py-2.5 pl-9 text-xs text-brand-text placeholder-brand-muted/70 outline-none transition-all font-medium"
              />
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar">
              {filteredUpcoming.map((apt) => (
                <div key={apt.id} className="p-4 bg-brand-bg/50 border border-brand-border rounded-2xl hover:border-brand-primary/30 transition-all group">
                    ...
                    <div className="flex items-start gap-2 max-w-[150px]">
                      <span 
                        className={cn(
                          ...
                          apt.status === 'confirmed' ? "bg-brand-primary" :
                          apt.status === 'completed' ? "bg-emerald-500" :
                          apt.status === 'cancelled' ? "bg-brand-danger" :
                          "bg-orange-500 animate-pulse"
                        )} 
                        title={
                          apt.status === 'confirmed' ? "Confirmado" :
                          apt.status === 'completed' ? "Concluído" :
                          apt.status === 'cancelled' ? "Cancelado" : "Pendente/Agendado"
                        }
                      />
                      <div className="min-w-0">
                        <p className="text-xs font-black text-brand-text uppercase tracking-tight truncate" title={apt.client?.name || apt.client_name || 'Cliente'}>
                          {apt.client?.name || apt.client_name || 'Cliente'}
                        </p>
                        <p className="text-[9px] text-brand-muted font-bold uppercase tracking-tighter truncate" title={apt.service?.name || apt.service_name || 'Serviço'}>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-brand-border/50">
                    <p className="text-[10px] font-black text-emerald-500 italic">R$ {(apt.service?.price || apt.price || 0).toFixed(2)}</p>
                    <button 
              ...
              {filteredUpcoming.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center opacity-35 text-center py-10">
                  <Clock className="w-8 h-8 mb-2 text-brand-muted" />
                  <p className="text-[10px] font-bold uppercase px-4 tracking-widest text-brand-muted">
                    {appointmentSearch ? 'Nenhum horário corresponde à busca' : 'Nenhum horário agendado'}
                  </p>
                </div>
              )}
            </div>
            ...
            <button 
              ...
              className="mt-4 w-full py-3 border border-brand-primary/20 rounded-xl text-[9px] font-black uppercase tracking-widest text-brand-primary hover:bg-brand-primary hover:text-white transition-all shadow-lg shadow-brand-primary/5 shrink-0"
            >
              Ver Agenda Completa
            </button>
          </div>
              ...
              <button 
                onClick={() => {
                  if (urgentInvoices.length > 0) {
                    setShowUrgentActionModal(true);
                  } else {
                    showToast('⚠️ Nenhuma cobrança urgente em atraso encontrada.', 'info');
                  }
                }}
      ...
      {/* Quick Urgent Billing Action Modal */}
      {showUrgentActionModal && urgentInvoices.length > 0 && (
        <div className="fixed inset-0 bg-brand-bg/85 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-brand-card border border-brand-border rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl relative"
          >
            <div className="p-6 border-b border-brand-border flex items-center justify-between bg-gradient-to-r from-brand-danger/5 to-transparent">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-brand-danger/10 rounded-xl flex items-center justify-center border border-brand-danger/20 text-brand-danger">
                  <Zap className="w-5 h-5 fill-current animate-pulse" />
                </div>
                <div>
                  <h3 className="font-display font-black text-brand-text uppercase text-xs sm:text-sm tracking-widest leading-none">Cobranças Urgentes</h3>
                  <p className="text-[9px] text-brand-muted uppercase font-black tracking-widest mt-1">Ações para pendências em atraso</p>
                </div>
              </div>
              <button 
                onClick={() => setShowUrgentActionModal(false)} 
                className="text-brand-muted hover:text-brand-text transition-all cursor-pointer"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              {/* Highlight of the first urgent invoice */}
              <div className="bg-brand-bg/50 rounded-2xl p-4 border border-brand-border relative overflow-hidden group">
                <span className="absolute right-3 top-3 text-[8px] sm:text-[9px] font-black tracking-widest uppercase bg-brand-danger/10 text-brand-danger px-2.5 py-1 rounded-full border border-brand-danger/20">
                  Mais Crítica 🚨
                </span>
                <span className="text-[10px] font-bold text-brand-muted uppercase tracking-wider block mb-1">Cliente / Valor</span>
                <p className="text-sm font-bold text-brand-text truncate uppercase">{urgentInvoices[0].client_name}</p>
                <div className="flex items-baseline gap-2 mt-2">
                  <span className="text-xl sm:text-2xl font-display font-black text-brand-danger">{formatCurrency(urgentInvoices[0].amount)}</span>
                  <span className="text-[9px] sm:text-[10px] font-mono text-brand-muted font-bold">(Venceu em {formatDate(urgentInvoices[0].due_date)})</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => {
                    ...
                    setShowUrgentActionModal(false);
                  }}
                  className="w-full bg-green-500 hover:bg-green-600 text-white font-black text-[11px] uppercase tracking-widest py-4 px-5 rounded-2xl flex items-center justify-between gap-2 transition-all active:scale-[0.98] cursor-pointer shadow-lg shadow-green-500/10"
                >
                  <div className="flex items-center gap-3">
                    <MessageSquare className="w-4 h-4 text-white" />
                    <span>WhatsApp Amigável Padrão</span>
                  </div>
                  <span className="text-[9px] opacity-80 uppercase tracking-widest font-black">Cobrar</span>
                </button>
                ...
                <button
                  onClick={() => {
                    setShowUrgentActionModal(false);
                    handleOpenAI(urgentInvoices[0]);
                  }}
                  className="w-full bg-brand-primary hover:bg-brand-primary-hover text-white font-black text-[11px] uppercase tracking-widest py-4 px-5 rounded-2xl flex items-center justify-between gap-2 transition-all active:scale-[0.98] cursor-pointer shadow-lg shadow-brand-primary/10 border border-brand-primary/20"
                >
                  <div className="flex items-center gap-3">
                    <Sparkles className="w-4 h-4 text-white" />
                    <span>Mensagem Personalizada com IA</span>
                  </div>
                  <span className="text-[9px] bg-white/20 px-2 py-0.5 rounded-full text-white uppercase font-black tracking-widest">IA</span>
                </button>
              </div>

              {/* More items notification */}
              {urgentInvoices.length > 1 && (
                <div className="p-3 bg-brand-bg/30 rounded-xl border border-brand-border text-center">
                  <p className="text-[9px] text-brand-muted font-bold uppercase tracking-wider">
                    ⚡ Há outras {urgentInvoices.length - 1} cobranças em atraso na tabela abaixo!
                  </p>
                </div>
              )}
            </div>

            <div className="p-4 bg-brand-bg/20 border-t border-brand-border text-center">
               <p className="text-[9px] text-brand-muted font-bold uppercase tracking-tighter">
                 Engajamento Inteligente de Alta Conversão
               </p>
            </div>
          </motion.div>
        </div>
      ...
      {selectedAppointment && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="w-full max-w-sm bg-brand-card border border-brand-border rounded-[2.5rem] p-8 shadow-2xl relative z-10 overflow-hidden text-left"
          >
            {/* Header / Light Glow */}
            ...
            <div className="flex items-center justify-between mb-6">
              <span className="text-[10px] font-black uppercase text-brand-primary bg-brand-primary/10 px-3 py-1.5 rounded-full tracking-widest flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" /> Detalhes do Horário
              </span>
              <button 
                onClick={() => setSelectedAppointment(null)}
                className="w-8 h-8 rounded-full bg-brand-bg/80 border border-brand-border flex items-center justify-center text-brand-muted hover:text-brand-text transition-colors font-bold text-xs"
              >
                ✕
              </button>
            </div>
            ...
            {/* Client Card */}
            <div className="p-5 bg-brand-bg/50 border border-brand-border rounded-2xl mb-5 space-y-3 text-left">
              <div>
                <p className="text-[9px] font-bold text-brand-muted uppercase tracking-widest mb-1">Cliente</p>
                <h4 className="text-lg font-display font-black text-brand-text uppercase leading-none">
                  {selectedAppointment.client?.name || selectedAppointment.client_name || 'Cliente'}
                </h4>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <span className={cn(
                  "text-[8px] font-black px-2.5 py-1 rounded-full border tracking-widest uppercase flex items-center gap-1.5 w-fit",
                  selectedAppointment.status === 'confirmed' ? "bg-brand-primary/10 text-brand-primary border-brand-primary/20" :
                  selectedAppointment.status === 'completed' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" :
                  selectedAppointment.status === 'cancelled' ? "bg-brand-danger/10 text-brand-danger border-brand-danger/20" :
                  "bg-orange-500/10 text-orange-500 border-orange-500/20"
                )}>
                  <div className={cn(
                    "w-1 h-1 rounded-full",
                    selectedAppointment.status === 'confirmed' ? 'bg-brand-primary' :
                    selectedAppointment.status === 'completed' ? 'bg-emerald-500' :
                    selectedAppointment.status === 'cancelled' ? 'bg-brand-danger' :
                    'bg-orange-500 animate-pulse'
                  )} />
                  {selectedAppointment.status === 'confirmed' ? 'Confirmado' :
                   selectedAppointment.status === 'completed' ? 'Concluído' :
                   selectedAppointment.status === 'cancelled' ? 'Cancelado' : 'Agendado/Pendente'}
                </span>
                
                {(selectedAppointment.client?.phone || selectedAppointment.client_phone) && (
                  <p className="text-xs font-mono text-brand-muted font-bold">
                    {selectedAppointment.client?.phone || selectedAppointment.client_phone}
                  </p>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 text-left">
              <div className="p-4 bg-brand-bg/30 border border-brand-border/40 rounded-xl">
                <p className="text-[8px] font-bold text-brand-muted uppercase tracking-widest mb-1">Procedimento</p>
                <p className="text-xs font-black text-brand-text uppercase truncate">
                  {selectedAppointment.service?.name || selectedAppointment.service_name || 'Serviço'}
                </p>
              </div>
              <div className="p-4 bg-brand-bg/30 border border-brand-border/40 rounded-xl">
                <p className="text-[8px] font-bold text-brand-muted uppercase tracking-widest mb-1">Valor</p>
                <p className="text-xs font-black text-emerald-500 italic">
                  R$ {(selectedAppointment.service?.price || selectedAppointment.price || 0).toFixed(2)}
                </p>
              </div>
              <div className="p-4 bg-brand-bg/30 border border-brand-border/40 rounded-xl">
                <p className="text-[8px] font-bold text-brand-muted uppercase tracking-widest mb-1">Data</p>
                <p className="text-xs font-black text-brand-text py-[1px]">
                  {formatDate(selectedAppointment.date)}
                </p>
              </div>
              <div className="p-4 bg-brand-bg/30 border border-brand-border/40 rounded-xl">
                <p className="text-[8px] font-bold text-brand-muted uppercase tracking-widest mb-1">Horário</p>
                <p className="text-xs font-black text-brand-primary uppercase py-[1px]">
            ...
            <div className="space-y-2 text-left">
               {(selectedAppointment.client?.phone || selectedAppointment.client_phone) && (
                 <button 
                  onClick={() => {
                    const phone = (selectedAppointment.client?.phone || selectedAppointment.client_phone).replace(/\D/g, '');
                    const clientName = selectedAppointment.client?.name || selectedAppointment.client_name || 'Cliente';
                    const serviceName = selectedAppointment.service?.name || selectedAppointment.service_name || 'Serviço';
                    const dateFormatted = selectedAppointment.date.split('-').reverse().join('/');
                    const msg = `Olá *${clientName}*! Passo para confirmar nosso horário de *${serviceName}* no dia *${dateFormatted}* às *${selectedAppointment.time}*.`;
                    window.open(`https://wa.me/55${phone}?text=${encodeURIComponent(msg)}`, '_blank');
                  }}
                  className="w-full bg-green-500 hover:brightness-110 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 transition-all text-xs tracking-widest uppercase shadow-xl shadow-green-500/10 border border-green-400/20"
                >
                  <MessageSquare className="w-4 h-4" /> Enviar Mensagem WA
                </button>
               )}
               ...
               <button 
                ...
                className="w-full bg-brand-primary hover:bg-brand-primary-hover text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 transition-all text-xs tracking-widest uppercase shadow-xl shadow-brand-primary/10"
              >
                Gerenciar na Agenda <ArrowUpRight className="w-4 h-4" />
              </button>

              <button 
                onClick={() => setSelectedAppointment(null)}
                className="w-full py-3 rounded-2xl font-bold text-brand-muted hover:bg-brand-bg/80 transition-all uppercase text-[10px] tracking-widest"
              >
                Fechar Detalhes
              </button>
            </div>
      ...
      {showPendingConfirmationsModal && (
        <div className="fixed inset-0 bg-brand-bg/85 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-brand-card border border-brand-border rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl relative animate-in fade-in"
          >
            <div className="p-6 border-b border-brand-border flex items-center justify-between bg-gradient-to-r from-brand-primary/5 to-transparent">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-brand-primary/10 rounded-xl flex items-center justify-center border border-brand-primary/20 text-brand-primary animate-pulse">
                  <CalendarCheck className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-display font-black text-brand-text uppercase text-xs sm:text-sm tracking-widest leading-none">Confirmar Horários</h3>
                  <p className="text-[9px] text-brand-muted uppercase font-black tracking-widest mt-1">Lembretes do Agenda Azul</p>
                </div>
              </div>
              <button 
                onClick={() => setShowPendingConfirmationsModal(false)} 
                className="text-brand-muted hover:text-brand-text transition-all cursor-pointer"
              >
                <X className="w-6 h-6" />
              </button>
            ...
            <div className="p-6 space-y-4 max-h-[350px] overflow-y-auto scrollbar-thin">
              {upcomingAppointments.filter(a => a.status === 'scheduled').map((apt) => (
                <div key={apt.id} className="bg-brand-bg/50 rounded-2xl p-4 border border-brand-border flex flex-col justify-between gap-3">
                  <div>
                    <p className="text-xs font-black text-brand-text truncate uppercase">{apt.client_name}</p>
                    <p className="text-[10px] text-brand-muted mt-0.5 font-sans">
                      {apt.service_name} • {apt.date.split('-').reverse().join('/')} às {apt.time}
                    </p>
                  </div>
                  <button
                    onClick={() => handleWhatsAppReminder(apt)}
                    className="w-full bg-brand-primary text-white text-[10px] font-black py-2.5 rounded-xl uppercase tracking-widest hover:brightness-110 transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >
            ...
            <div className="p-6 border-t border-brand-border bg-brand-bg/30">
              <button
                onClick={() => setShowPendingConfirmationsModal(false)}
                className="w-full py-3 bg-brand-card hover:bg-brand-bg/80 border border-brand-border text-brand-text font-black rounded-2xl transition-all uppercase text-[10px] tracking-widest cursor-pointer"
              >
                Fechar Resumo
              </button>
            </div>
