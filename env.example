...
export default function Reports({ userId, subscription }: { userId: string, subscription: Subscription | null }) {
  ...
  const isPaid = isAdmin || subscription?.email?.toLowerCase() === 'ronilsonaugustomg@gmail.com' || subscription?.plano === 'business' || subscription?.plano === 'premium';

  useEffect(() => {
    fetchReportData();
    const checkSession = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user?.email?.toLowerCase() === 'ronilsonaugustomg@gmail.com') {
        setIsAdmin(true);
      }
    };
    checkSession();
  }, [userId, selectedPeriod, subscription]);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      setShowPeriodMenu(false);
      // Aguarda 600ms para o React, os gráficos Recharts e as SVGs se ajustarem ao novo layout de 1200px
      ...
      const canvas = await html2canvas(reportRef.current, {
        scale: 2, // Excelência de resolução e nitidez de fontes
        useCORS: true,
        allowTaint: true,
        ...
        scrollX: 0,
        scrollY: 0,
        windowWidth: 1200, // Garante renderização em alta resolução desktop
        onclone: (doc) => {
          ...
          // Reescreve as stylesheets do documento CLONADO de forma isolada, sem afetar o host
          try {
          ...
          // Remove botões e elementos indesejáveis/flutuantes do PDF clonado
          const elementsToHide = doc.querySelectorAll('button, .print\\:hidden, .hidden-in-pdf, [class*="fixed"], [class*="z-50"]');
          elementsToHide.forEach(el => (el as HTMLElement).style.display = 'none');
          
          const container = doc.getElementById('report-container-pdf');
          if (container) {
      ...
      // Páginas subsequentes se o conteúdo transbordar uma folha A4
      while (heightLeft > 0) {
  ...
  return (
    <motion.div 
      ...
      className={`p-4 sm:p-8 space-y-8 ${isPrinting ? 'w-[1200px] max-w-[1200px] min-w-[1200px] bg-[#0a0f16]' : ''}`}
      ref={reportRef}
